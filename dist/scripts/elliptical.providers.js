            ...
            for(var key in query){
                if(query.hasOwnProperty(key)){
                    ...
                    if(key.indexOf('sw_')===0){
                        ...
                        str += (checksum > 0) ? " and startswith(" + prop + ",'" + decodeURIComponent(query[key]) + "')"  : "startswith(" + prop + ",'" + decodeURIComponent(query[key]) + "')";
                        checksum++;
                    }else if(key.indexOf('c_')===0){
                        prop=key.substring(2);
                        str += (checksum > 0) ? " and substringof(" + prop + ",'" + decodeURIComponent(query[key]) + "')"  : "substringof(" + prop + ",'" + decodeURIComponent(query[key]) + "')";
                        checksum++;
                    }else if(key.indexOf('ew_')===0){
                        prop=key.substring(3);
                        str += (checksum > 0) ? " and endswith(" + prop + ",'" + decodeURIComponent(query[key]) + "')"  : "endswith(" + prop + ",'" + decodeURIComponent(query[key]) + "')";
                        checksum++;
                    }else if(key.indexOf('$')!==0){
                        str += (checksum > 0) ? " and " + key + " eq '" + decodeURIComponent(query[key]) + "'" : key + " eq '" + decodeURIComponent(query[key]) + "'";
                        checksum++;
                    }
                }
            }
